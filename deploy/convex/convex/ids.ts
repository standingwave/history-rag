/* Chunk ids computed on the server, so an optimistic task row carries the
   same id `sources/tasks.py` will give the real chunk: "tasks:" +
   sha256(vault ⏎ NUL ⏎ normalised text)[:26]. Mutations have no
   crypto.subtle, hence a small sha256 of our own. */
...
/* `sources/tasks.py` _norm: whitespace collapsed, lower-cased. */
export function normTask(text: string): string {
